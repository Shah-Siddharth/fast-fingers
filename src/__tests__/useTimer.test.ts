...
function setup(currentWord: string, difficultyFactor: number, gameState: gameStateType) {
  const { result } = renderHook(() => useTimer(currentWord, difficultyFactor, gameState));
...
describe("useTimer hook", () => {
  test("hook should contain correct initial values", () => {
    ...
    const { maxTimeForWord, gameOver } = setup(currentWord, difficultyFactor, 'PLAYING');
